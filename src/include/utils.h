...
namespace utils
{
	...
	inline std::string getSavedGamesDir() {
		std::string systemDrive = get_env_var("SYSTEMDRIVE");
		std::string homePath = get_env_var("HOMEPATH");
		std::string path = systemDrive + homePath + "\\Saved Games";
		return path;
	}

	inline std::string getLostMarblesWriteDir() {
		std::string path = getSavedGamesDir() + "\\LostMarbles";
		return path;
	}
}
